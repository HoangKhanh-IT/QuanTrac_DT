...
$(function() {
    $.getJSON("obstyles", function(data_obstyles) {
        $('#loaihinh').jstree({
            'core': {
                /*** Load lần đầu sẽ tự động đóng lại ***/
            ...
            var obj_node = data.instance.get_node(data.node, true);
            if (obj_node) {
...
function dom_loctype_option() {
    $.getJSON("loctypes", function(data_locType) {
        $('#locType')
            .append($("<option></option>")
...
function dom_location_option() {
    $.getJSON("locations", function(data_location) {
        $('#location')
            .append($("<option></option>")
