                        ...
                        <div class="col-sm-12">
                            ...
                            <div class="card card-primary">
                                <div class="card-header">
                                    <h3 class="card-title">Cập nhật địa danh</h3>
                                </div>
                                <!-- /.card-header -->
                                <!-- form start -->
                                <form method="post" action="{{ route('Place.edit',$Location->id) }}">
                                    @csrf
                                    <div class="card-body">
                                        <div class="form-group">
                                            <label>Tên địa danh (<span style="color: red;">*</span>)</label>
                                            <input name="name" id="name" type="text" class="form-control"
                                                placeholder="Tên địa danh..." value="{{ $Location->name }}">
                                        </div>
                                            ...
                                            <select class="js-example-basic-single form-control" id="locationtypeid" name="locationtypeid">
                                               <!--  <option value="">--Chọn loại địa danh--</option> -->
                                                @foreach($LocationTypes as $LocationType)
                                                    <option {{ $Location->locationtypeid == $LocationType->id ?"Selected":""}} value="{{$LocationType->id}}">
                                                        {{$LocationType->name}}
                                                    </option>
                                                @endforeach
                                            </select>
                                        ...
                                        <div class="form-group">
                                            <label>Mô tả</label>
                                            <input name="note" id="note" type="text" class="form-control" placeholder="Mô tả ..." value="{{ $Location->note }}">
                                        </div>
