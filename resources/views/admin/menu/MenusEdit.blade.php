                    ...
                    <div class="row mb-12">
                        ...
                        <div class="col-sm-12">
                            ...
                            <div class="card card-primary">
                                ...
                                <!-- /.card-header -->
                                <!-- form start -->
                                <form method="post" action="{{ route('menu.edit',$MenuItem->id) }}">
                                    @csrf
                                    <div class="card-body">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select class="form-control select2 select2-hidden-accessible"
                                                ...
                                                <?php  $value = $nhomCN->id.'_'.$nhomCN->code; $valueItem = $MenuItem->parent_id;  ?>
                                                <option
                                                    <?php  if(str_contains($value, $valueItem) !== false){ echo 'selected'; } ?>
                                                    value="{{ $nhomCN->id.'_'.$nhomCN->code }}">{{ $nhomCN->name }}
                                                </option>
                                                @endforeach
                                            </select>
                                        </div>
                                            ...
                                            <table class="table table-bordered" id="dynamicTable">
                                                <tr>
                                                    ...
                                                    @foreach ($permissions as $key => $permission)
                                                        <?php $j=0 ?>
                                                        <tr>
                                                            <input id="prodId" name="addmore[{{ $key }}][id]"
                                                                type="hidden"
                                                                value="{{ $permission->id }}">
                                                        <td><input type="text" name="addmore[{{ $key }}][name]"
                                                            value="{{ $permission->name }}" placeholder="Action"
                                                            class="form-control" /></td>
                                                        <td><input type="text" name="addmore[{{ $key }}][display_name]"
                                                            value="{{ $permission->display_name }}" placeholder="Mô tả"
                                                            class="form-control" /></td>
                                                        <td><button type="button" class="btn btn-danger remove-tr"><i class="fas fa-minus-square"></i></button></td>
                                                        <?php $j++ ?>
                                                    @endforeach
...
<script type="text/javascript">
    ...
    $("#add").click(function(){
        ++i;
        $("#dynamicTable").append('<tr><td><input  name="addmore['+i+'][id]" type="hidden" value="moi"><input type="text" name="addmore['+i+'][name]" placeholder="Action" class="form-control" /></td><td><input type="text" name="addmore['+i+'][display_name]" placeholder="Mô tả" class="form-control" /></td><td><button type="button" class="btn btn-danger remove-tr"><i class="fas fa-minus-square"></i></button></td></tr>');
    });
