                        ...
                        <div class="col-sm-12">
                            ...
                            <div class="card card-primary">
                                <div class="card-header">
                                    <h3 class="card-title">Sửa nhóm người dùng</h3>
                                </div>
                                <!-- /.card-header -->
                                <!-- form start -->
                                <form method="post" action="{{ route('role.edit',$role->id) }}">
                                    ...
                                    <div class="card-body">
                                        ...
                                        <input name="tennhom" id="tennhom" type="text" class="form-control" value="{{ $role->name }}"
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-sm-4">
                                                <div class="form-group">
                                                <label>{{ $chucnang->name }}</label>
                                                @foreach($chucnang->permissions as $key =>
                                                $permission)
                                                         <div class="form-check">
                                                             <input class="form-check-input" type="checkbox"
                                                         name="permissions[]" id="permissions[]"
                                                         value="{{ $permission->id }}"
                                                          @foreach($chucnangnhoms as $key =>
                                                          $chucnangnhom)
                                                                @if ($chucnangnhom->permission_id == $permission->id)
                                                                    checked="checked"
                                                                @endif
                                                          @endforeach
                                                         >
                                                             <label class="form-check-label">
                                                                 {{ $permission->display_name }}({{ $permission->name }})</label>
                                                         </div>
                                                @endforeach
                                                </div>
                                            </div>
                                    ...
                                    <div class="card-footer">
                                        ...
                                        <a href="{{ route('role') }}" class="btn btn-default float-right">
                                            Quay lại
                                        </a>
                                    </div>
                                </form>
